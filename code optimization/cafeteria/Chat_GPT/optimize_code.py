...
class Track:
    ...
    def place_order(self, order:'Coffee'):
        ...
        if self.beans >= order.espresso // 5 and self.milk >= order.milk:
            self.beans -= order.espresso // 5
            self.milk -= order.milk
        else:
            return "Unfortunately, we don't have enough ingredients."
        self.orders.append(order)
        ###
        self.t_revenue += order.price
        self.t_milk += order.milk
        self.t_beans += order.espresso//5
        ###
        order.is_paid = True
        return 'Done!'
